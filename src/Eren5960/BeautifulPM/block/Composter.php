...
class Composter extends Transparent {
    ...
    public function __construct(
        ...
        parent::__construct(
            $idInfo,
            $name,
            $breakInfo ?? new BlockBreakInfo(0.6, BlockToolType::AXE, 0, 0.6)
        );
    }
    ...
    public function readStateFromData(int $id, int $stateMeta): void {
        $this->composter_fill_level = BlockDataSerializer::readBoundedInt(
    ...
    public function onInteract(
        ...
        if ($this->composter_fill_level >= 8) {
            ...
            if (
                $downTile instanceof Hopper &&
                $downTile->getInventory()->canAddItem(VanillaItems::BONE_MEAL())
            ) {
                $downTile->getInventory()->addItem(VanillaItems::BONE_MEAL());
            } else {
                $this->getPos()
                    ->getWorld()
                    ->dropItem($this->pos->up(), VanillaItems::BONE_MEAL());
            }
            $this->pos
                ->getWorld()
                ->broadcastPacketToViewers(
                    ...
                    LevelSoundEventPacket::create(
                        LevelSoundEventPacket::SOUND_BLOCK_COMPOSTER_EMPTY,
                        $this->pos
                    )
                );
            return true;
            ...
            if (rand(0, 99) <= $rate) {
                $this->pos
                    ...
                    ->broadcastPacketToViewers(
                        $this->pos,
                        LevelSoundEventPacket::create(
                            LevelSoundEventPacket::SOUND_BLOCK_COMPOSTER_FILL_SUCCESS,
                            $this->pos
                        )
                    );
                ...
                if ($this->composter_fill_level >= 7) {
                    ...
                    $this->pos
                        ->getWorld()
                        ->scheduleDelayedBlockUpdate($this->pos, 15);
                } else {
                    $this->pos->getWorld()->setBlock($this->pos, $this);
                }
            } else {
                $this->pos
                    ...
                    ->broadcastPacketToViewers(
                        $this->pos,
                        LevelSoundEventPacket::create(
                            LevelSoundEventPacket::SOUND_BLOCK_COMPOSTER_FILL,
                            $this->pos
                        )
                    );
            }
    ...
    public function onScheduledUpdate(): void {
        ...
        $this->pos
            ...
            ->broadcastPacketToViewers(
                $this->pos,
                LevelSoundEventPacket::create(
                    LevelSoundEventPacket::SOUND_BLOCK_COMPOSTER_READY,
                    $this->pos
                )
            );
    }
